...
function []=main_function_lead(qi, ui, K, z, p, sv, ani)
    ...
    u2n(i,:) =...
        ...
        hn = ...
            ...
            elseif Launch_r == true
                ...
                ekr(i+2) = qn(i,5);
                Vr(i+1) = (1-p*dt)*Vr(i) + K*(ekr(i+2) + (z*dt-1)*ekr(i+1));
                %%%%
            ...
            elseif Launch_l == true
                ...
                ekl(i+2) = -qn(i,5);
                Vl(i+1) = (1-p*dt)*Vl(i) + K*(ekl(i+2) + (z*dt-1)*ekl(i+1));
                %%%%
...
set(findall(gcf,'type','line'),'linewidth',2);
if sv
saveas(gcf, sprintf("images/%s_%d_z_%.4f_p_%.4f.png", datestr(now, 'mmdd_HHMMSSFFF'), K, z, p));
end
